...
void* subThread(void* arg)
{
    ...
    struct epoll_event readSockets[100]; //
    int waitRet;
    int ret;
    while (1)
    {
        printf("%ld子线程等待连接\n", pthread_self());
        waitRet = epoll_wait(instanceId, readSockets, 100, -1); //
...
void runServer()
{
    ...
    myListen(mainSocket, 100);
    struct sockaddr_in peerAddr;
    int ret = 0; // 保存返回值
    intptr_t newSfd;
    intptr_t instances[4];
    for (int i = 0; i < 4; i++)
    {
    ...
    while (1)
    {
        ...
        else if (waitRet > 0) //
        {
            ...
            event.events = EPOLLIN; // 子套接字默认水平模式并且监听可读事件
