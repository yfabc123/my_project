...
int TcpSocket::myConnect(int paraTcpSocket, const char *pararIp, short paraPort, struct sockaddr_in *paraPeerAddr)
{
    int ret; // 默认连接失败
...
// 绑定我方地址
int TcpSocket::myBind(int paraTcpSocket,const char *pararIp, short paraPort, struct sockaddr_in *paraMyAddr)
{
    int ret; // 默认连接失败
...
// 设置最大连接队列
int TcpSocket::myListen(int paraTcpSocket, int paraQueueLength)
{
...
int TcpSocket::myAccept(int paraTcpSocket, struct sockaddr_in *paraPeeAddr)
{
...
int TcpSocket::mySend(int paraTcpSocket, void *buf, size_t bufSize, int flag)
{
...
// 设置为MSG_NOSIGNAL  忽略长度为0时的断开信号!
int TcpSocket::myRecv(int paraTcpSocket, void *buf, size_t bufSize, int flag)
{
...
int TcpSocket::setNonblocking(int paraTcpSocket)
{
...
1:正确接收数据
 */
int TcpSocket::myUpgradeRecv(int paraTcpSocket, void *paraData, size_t paraMaxByte, int paraFlag)
{
